 ...
 * circumradius. A 1000 m cell is therefore 1000 m from its northern to its
...
/**
 * Shrink a rendered hexagon around its Web Mercator centre.
 *
 * This deliberately operates on polygon geometry rather than the logical grid:
 * cell assignment, axial coordinates, and cell IDs therefore remain unchanged.
 * A scale of 1 preserves the original polygon; values below 1 create a visual
 * gap between neighbouring rendered cells.
 */
export function shrinkHexPolygon(
  ...
  if (renderScale <= 0 || renderScale > 1) {
    throw new RangeError('Hex render scale must be greater than zero and at most one.');
  }
