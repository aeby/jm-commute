...
function clockTimeSeconds(value: string): number {
  ...
  return (hours as number) * 3_600 + (minutes as number) * 60 + (seconds as number);
}
...
function parseRoutingPolicy(
  ...
  if (value.virtualTransfersEnabled !== true && value.virtualTransfersEnabled !== false) {
    return invalid(
      source,
...
function parseServiceDate(value: unknown, source: string): string {
  ...
  if (
    ...
    return invalid(source, 'source.serviceDate', 'expected a real calendar date');
  }
  return value;
}
