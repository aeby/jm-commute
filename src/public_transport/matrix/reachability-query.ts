...
import type { TransitReachabilityQuery } from './travel-time-row';
...
/** Creates the locality-level RAPTOR query used by matrix compilation. */
export function createRaptorReachabilityQuery(
  options: RaptorReachabilityQueryOptions,
): TransitReachabilityQuery {
  validateOptions(options);
  const stopCount = options.network.sourceStopIds.length;
  ...
  return (originLocalityId: LocalityId, maxTravelMinutes: number) => {
    ...
    const result = runRaptorFastestWindow(options.network, {
