...
const isNonOvertaking = (
  chain: readonly GroupedConcreteTrip[],
  stopCount: number,
): boolean => {
  for (let tripIndex = 1; tripIndex < chain.length; tripIndex += 1) {
    const previous = chain[tripIndex - 1];
    const current = chain[tripIndex];
    ...
    for (let stopIndex = 0; stopIndex < stopCount; stopIndex += 1) {
      ...
      const previousArrival = previous.stopTimes[timeIndex];
      const previousDeparture = previous.stopTimes[timeIndex + 1];
      const currentArrival = current.stopTimes[timeIndex];
      const currentDeparture = current.stopTimes[timeIndex + 1];
      if (
        previousArrival === undefined ||
        previousDeparture === undefined ||
        currentArrival === undefined ||
        currentDeparture === undefined
      ) {
        return false;
      }
      if (
...
describe('network splitOvertakingTrips', () => {
  ...
  it('produces only non-overtaking chains', () => {
    const chains = splitOvertakingTrips(
      [
        trip('a', [
          [100, 100],
    ...
    expect(chains.every((chain) => isNonOvertaking(chain, 2))).toBe(true);
  });
