...
describe('prepareFixedDayRoutingData', () => {
  it('writes the configured fixed-day subset as deterministic NDJSON', async () => {
    ...
    expect(manifest).toEqual({
      ...
      routingWindowStart:
        PROJECT_CONFIG.publicTransport.referenceScenario.morningWindow.start,
      routingWindowEnd:
        PROJECT_CONFIG.publicTransport.referenceScenario.morningWindow.end,
  ...
  it('preserves complete sequences, conditional pickups, route IDs, and frequency templates', async () => {
    const workspace = await createFixtureWorkspace();
    ...
    expect(scheduledExact?.routeId).toBe('route-bus');
  ...
  it('writes one valid object per line with stable property ordering and matching counts', async () => {
    ...
    expect(lines[0]).toMatch(
      /^\{"tripId":.+,"routeId":.+,"stopTimes":.+,"frequencyWindows":.+}$/,
    );
    expect(lines[0]).toContain(
      '"stopId":"stop-1","arrivalTimeSeconds":',
    );
    expect(tripsBytes.toString('utf8')).not.toMatch(
      /"(?:routeType|stopSequence|exactTimes)":/,
    );
  ...
  it('keeps opaque route IDs without loading routes.txt', async () => {
    const workspace = await createFixtureWorkspace();
    ...
    const { trips: preparedTrips } = await readOutput(workspace);
    expect(
      preparedTrips.find(({ tripId }) => tripId === 'scheduled-exact')
        ?.routeId,
    ).toBe('unknown-route');
  });
