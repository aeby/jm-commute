...
const manifest = (
  ...
  const scheduledTripCount = trips.filter(
  ...
  return {
    ...
    scheduledTripCount,
    frequencyTripCount: trips.length - scheduledTripCount,
    stopTimeCount: trips.reduce(
      (count, trip) => count + trip.stopTimes.length,
      0,
    ),
    frequencyWindowCount: trips.reduce(
      (count, trip) => count + trip.frequencyWindows.length,
      0,
    ),
  };
};
